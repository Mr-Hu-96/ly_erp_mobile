<template>
  <view class="">
    <wd-popup v-model="show" position="bottom" closable>
      <view class="text-center font-bold lh-10">销售订单选择</view>
        ...
        <z-paging ref="paging" :fixed="false" v-model="dataList" @query="queryList">
                ...
                <wd-drop-menu>
                  <wd-drop-menu-item title="筛选" ref="dropMenu">
                    <wd-form ref="queryFormRef" :model="queryParams">
                      <wd-input label="客户名称" v-model="queryParams.custName" />
          ...
          <wd-card v-for="(item, index) in dataList" :key="index">
            ...
            <wd-cell-group border @click="() => $emits('select', item)">
              <wd-cell title="单据编号" :value="item.salesOrderCode" />
              <wd-cell title="客户名称" :value="item.custName" />
            </wd-cell-group>
          </wd-card>
        </z-paging>
      </view>
...
import { listOrderApi } from '@/api/sales/order'
