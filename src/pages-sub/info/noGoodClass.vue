  ...
  <view>
    <z-paging ref="paging" v-model="dataList" @query="queryList">
      ...
      <wd-card v-for="(item, index) in dataList" :key="index">
        ...
        <wd-cell-group border>
          <wd-cell title="时间" :value="item.date" />
          <wd-cell title="不良品项编号" :value="item.defectItemCode" />
          <wd-cell title="不良品项名称" :value="item.defectItemName" />
          <wd-cell title="不良品数量" :value="item.qty" />
          <wd-cell title="占比" :value="item.rate" />
        </wd-cell-group>
      </wd-card>
    </z-paging>
  </view>
...
import { statisticsDefectListApi } from '@/api/produce/report'
const dataList = ref([])
...
function queryList(pageNum, pageSize) {
  ...
  console.log(params)
  statisticsDefectListApi(params).then((res) => {
